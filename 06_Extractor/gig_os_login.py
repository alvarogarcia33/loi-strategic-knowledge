...
def _goto_session_check_page(page: Page, gig_os: PlatformConfig, config: AppConfig) -> None:
    url = gig_os.base_url or gig_os.login_url
...
def _is_logged_in(page: Page, gig_os: PlatformConfig, logger: logging.Logger) -> bool:
    if gig_os.login_success_selector and _selector_visible(page, gig_os.login_success_selector):
        logger.info("GIG-OS login success selector detected.")
        return True
    if gig_os.logout_selector and _selector_visible(page, gig_os.logout_selector):
        logger.info("GIG-OS logout selector detected.")
        return True

    password_visible = any(_selector_visible(page, selector) for selector in PASSWORD_SELECTORS)
    current_url = page.url.lower()
    login_url = gig_os.login_url.lower()
    still_on_login_url = current_url.rstrip("/") == login_url.rstrip("/")

    if not password_visible and not still_on_login_url:
        logger.info("GIG-OS login inferred from URL change and hidden password field.")
        return True

    logger.info(
        "GIG-OS login not confirmed. current_url=%s password_visible=%s still_on_login_url=%s",
        page.url,
        password_visible,
        still_on_login_url,
    )
...
def _wait_for_login_confirmation(
    ...
    while True:
        ...
        elif form_disappeared:
            condition = "form_disappeared"
        elif url_changed and not login_form_visible:
            condition = "url_changed_and_form_hidden"
        elif url_changed and spinner_disappeared:
            condition = "url_changed_and_spinner_finished"
        elif url_changed:
