...
from olympia_news import extract_olympia_news
...
class OlympiaDailyUpdateResult:
    run_id: int
    started_at: str
    finished_at: str
...
def run_olympia_daily_update(config: AppConfig, logger: logging.Logger) -> OlympiaDailyUpdateResult:
    started_at = datetime.now().astimezone().isoformat()
    run_id = _start_run(config.database_path, mode="daily_update", platform=PLATFORM)
    _add_run_event(config.database_path, run_id, "INFO", PLATFORM, None, "Daily Olympia update started.")

    try:
        summary = extract_olympia_news(config=config, logger=logger, limit=None)
        ...
        return OlympiaDailyUpdateResult(
            run_id=run_id,
            started_at=started_at,
            finished_at=finished_at,
        ...
        finished_at = datetime.now().astimezone().isoformat()
        _add_run_event(config.database_path, run_id, "ERROR", PLATFORM, None, f"Daily Olympia update failed: {exc}")
        _finish_run(
            database_path=config.database_path,
            run_id=run_id,
            status="failed",
...
def _write_daily_reports(
    ...
    report_json_path = report_root / f"olympia_daily_update_{day_stamp}_{time_stamp}.json"
    report_md_path = report_root / f"olympia_daily_update_{day_stamp}_{time_stamp}.md"
    ...
    if summary.duplicate_urls:
        lines.extend(f"- `{url}`" for url in summary.duplicate_urls)
    else:
        lines.append("- None")
