...
DASHBOARD_URL = "https://olympia-lab.com/es/dashboard/"
CONTENT_KEYWORDS = (
    "Noticias",
    "Novedades",
    "News",
...
def discover_olympia_structure(config: AppConfig, logger: logging.Logger) -> OlympiaStructureReport:
    ...
    logger.info("Starting Olympia structure discovery.")
    ...
    with sync_playwright() as playwright:
        ...
        try:
            ...
            if content_sections:
                ...
                logger.info(
                    "Opening first candidate content section for screenshot: %s -> %s",
                    section.text,
                    section.href,
                )
                ...
                logger.info("No named content section found for section screenshot.")

            report = OlympiaStructureReport(
            ...
            context.close()
            browser.close()
            logger.info("Olympia structure discovery browser closed.")

    json_path = config.project_root / "data" / "olympia_structure_report.json"
    markdown_path = config.output_root / "08_Reportes" / "olympia_structure_report.md"
    _write_json_report(report, json_path, logger)
    _write_markdown_report(report, markdown_path, logger)
    logger.info("Olympia structure discovery completed.")
    return report
...
def _capture_screenshot(
    ...
    path = screenshot_dir / f"olympia_structure_{name}_{timestamp}.png"
    page.screenshot(path=str(path), full_page=True)
    logger.info("Olympia structure screenshot saved: %s", path)
...
def _detect_menu_candidates(page: Page, logger: logging.Logger) -> list[MenuCandidate]:
    ...
    logger.info("Detected %s possible main menu containers.", len(candidates))
    return candidates
    ...
    for selector in button_selectors:
        try:
            ...
            if locator.count() > 0 and locator.is_visible(timeout=500):
                logger.info("Trying to expand main menu with selector: %s", selector)
                locator.click(timeout=2_000)
                return
        except Exception as exc:
            logger.debug("Menu expansion selector failed: %s (%s)", selector, exc)
...
def _content_section_sort_key(link: InternalLink) -> tuple[int, int, int, str]:
    ...
    spanish_news = "/es/news/" in link.href or link.href.rstrip("/").endswith("/es/news")
    return (
        0 if exact_keyword else 1,
        0 if spanish_news else 1,
        len(link.text),
        link.text.lower(),
    )
...
def _looks_like_article_url(href: str) -> bool:
    ...
    article_markers = ("/post/", "/article/", "/articles/")
    return any(marker in parsed_path for marker in article_markers)
...
def _build_recommended_strategy(
    ...
    strategy = [
        "Use the authenticated Playwright storage state and start from the dashboard URL.",
        "Use the detected internal links as the seed list; keep filtering to the same origin.",
        "Prioritize content-section URLs whose text or path matches Noticias/Novedades/News/Updates/Blog/Articles/Announcements.",
    ]
    if content_sections:
        strategy.append(
            "For the first extraction pass, visit each candidate section and detect repeated article-card links before opening individual articles."
        )
        strategy.append(
            "Keep a conservative incremental mode: revisit candidate sections daily and stop when URLs already exist in SQLite."
        )
    elif internal_links:
        strategy.append(
            "No named content section was found; next pass should inspect dashboard network calls and visible menu interactions."
        )
    else:
        strategy.append(
            "No internal links were detected; next pass should inspect authenticated app routes and API traffic."
        )
    strategy.append("Do not download article bodies until the section selectors and pagination behavior are confirmed.")
    return strategy


def _write_json_report(report: OlympiaStructureReport, path: Path, logger: logging.Logger) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(
        json.dumps(asdict(report), ensure_ascii=False, indent=2),
        encoding="utf-8",
    )
    logger.info("Olympia JSON structure report saved: %s", path)


def _write_markdown_report(report: OlympiaStructureReport, path: Path, logger: logging.Logger) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    lines = [
        "# Olympia Structure Report",
        "",
    ...
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
    logger.info("Olympia Markdown structure report saved: %s", path)
